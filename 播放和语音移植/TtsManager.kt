...
class TtsManager(context: Context) {
    ...
    companion object {
        private const val TAG = "HsiaoWear-TTS"
    }
    ...
    private fun warmupTts() {
        try {
            val warmupParams = Bundle().apply {
                putFloat(TextToSpeech.Engine.KEY_PARAM_VOLUME, 0f) // 静音
            }
    ...
    fun speak(text: String) {
        if (!isInitialized) {
            ...
            val callerStack = Thread.currentThread().stackTrace.take(5).joinToString("\n") { "  at ${it.className}.${it.methodName}(${it.fileName}:${it.lineNumber})" }
            Log.d(TAG, "speak 调用栈:\n$callerStack")
            onInitCallback = {
                speakInternal(text)
                onInitCallback = null
    ...
    private fun speakInternal(text: String) {
        ...
        Log.d(TAG, "当前 TTS 引擎: ${tts?.defaultEngine}, isSpeaking=$isSpeaking, isInitialized=$isInitialized")

        // 使用 Bundle 而非 null，兼容部分引擎
        val params = Bundle()
