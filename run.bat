...
:: 第一步：编译
:: ============================
echo %COLOR_YELLOW%[1/5] 正在编译 Debug APK ...%COLOR_RESET%
echo.

call gradlew.bat assembleDebug
if %ERRORLEVEL% neq 0 (
    echo.
    echo %COLOR_RED%[失败] 编译出错，请检查上面的错误信息%COLOR_RESET%
    echo 按任意键退出 & pause >nul
    exit /b %ERRORLEVEL%
)
echo.
echo %COLOR_GREEN%[完成] 编译成功%COLOR_RESET%

:: ============================
:: 第二步：查找生成的 APK
:: ============================
echo %COLOR_YELLOW%[2/5] 正在查找生成的 APK 文件 ...%COLOR_RESET%

set APK_PATH=
for /f "delims=" %%i in ('dir /s /b app\build\outputs\apk\debug\*.apk 2^>nul') do set "APK_PATH=%%i"

if "%APK_PATH%"=="" (
    echo %COLOR_RED%[失败] 未找到生成的 APK 文件%COLOR_RESET%
    pause
    exit /b 1
)
echo %COLOR_GREEN%[完成] APK: %APK_PATH%%COLOR_RESET%

:: ============================
:: 第三步：安装
:: ============================
echo %COLOR_YELLOW%[3/5] 正在安装到已连接的设备 ...%COLOR_RESET%

adb install -r "%APK_PATH%"
if %ERRORLEVEL% neq 0 (
    echo.
    echo %COLOR_RED%[失败] 安装失败%COLOR_RESET%
    echo   可能原因：
    echo     1. 没有连接 Android 设备或模拟器
    echo     2. 设备未开启 USB 调试
    echo     3. 已有相同签名冲突（卸载旧版本重试）
    echo.
    echo 请先用 adb devices 检查设备连接状态
    echo 按任意键退出 & pause >nul
    exit /b %ERRORLEVEL%
)
echo %COLOR_GREEN%[完成] 安装成功%COLOR_RESET%

:: ============================
:: 第四步：清空旧日志，然后启动 App
:: ============================
echo %COLOR_YELLOW%[4/5] 正在启动 App ...%COLOR_RESET%
...
:: ============================
echo.
echo %COLOR_YELLOW%[5/5] 正在显示日志（仅显示本应用相关）%COLOR_RESET%
echo %COLOR_CYAN%------------------------------------------------%COLOR_RESET%
